...
<Columns cols={2}>
  ...
  <Card title="Environments" icon="box">
    `devic environments …` — the machine, its secrets, its snapshot and who is connected to it.
  </Card>
  <Card title="Sandboxes" icon="terminal">
    `devic sandbox start | exec | stop`, plus `ls`, `cat` and `write` on a real Linux machine.
  </Card>
</Columns>
...
An agent can run itself. There is no external cron to set up, and the three
...
Environments and sandboxes take the environment by id **or by name**, and the
sandbox commands find the live session on their own after `start`.
...
devic sandbox stop "Reporting box"          # this is what saves the snapshot
```

Secrets belong in `--env`: they are encrypted at rest, injected into the machine
as ordinary environment variables, and always read back masked. `--env` merges
over what is stored, so adding one variable does not destroy the others.
